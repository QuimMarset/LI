...
filledInputValues. %forçar que sigui true al final per poder seguir
                                    %tota x que compleixi cond(x) Lits te x-1-1-1,x-1-1-2 ... x-1-1-9
...
atMost(K,Lits):-   % l1+...+ln <= k:  in all subsets of size k+1, at least one is false:
    negateAll(Lits,NLits), %nega tots els literals i agafa un subconjunt de k + 1 i escriu la clausula
...
expressOr( Var, Lits ):- member(Lit,Lits), negate(Lit,NLit), writeClause([ NLit, Var ]), fail.   %Transf tseitin
