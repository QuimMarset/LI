...
%   2. works-G-H: "gangster G works at hour H"

writeClauses(K):- 
    initClauseGeneration,
    eachHourEachGangsterAtMostOneTask,
    eachTaskAndHourNumberOfGangsters,
    eachGangsterAndTwoHoursNotDifferentTasks,
    relateVariables,
    eachGangsterMaxKConsecutiveHours(K),
    true,!.
...
eachTaskAndHourNumberOfGangsters:- task(T), hour(H), needed(T,H,N), findall(does-G-T-H, available(G,H), Lits), atLeast(N,Lits), fail.
eachTaskAndHourNumberOfGangsters.

eachGangsterAndTwoHoursNotDifferentTasks:- gangster(G), hour(H1), hour(H2), H1 is H2-1, available(G,H1), available(G,H2), task(T1), task(T2), T1 \= T2, writeClause([\+does-G-T1-H1,\+does-G-T2-H2]), fail.
eachGangsterAndTwoHoursNotDifferentTasks.

relateVariables:- available(G,H), task(T), writeClause([\+does-G-T-H,works-G-H]), fail.
relateVariables.

createClause(G,H,H,[\+works-G-H]).
createClause(G,H,HK,[\+works-G-H|L]):-  H2 is H+1 , H2 =< HK, createClause(G,H2,HK,L),!.

eachGangsterMaxKConsecutiveHours(K):- available(G,H), HK is H + K, hour(HK), createClause(G,H,HK,L), writeClause(L), fail.
eachGangsterMaxKConsecutiveHours(_).
...
maxConsecutiveHours(M,K):- between(0,71,N), K is 72-N, gangster(G), findall(H, member(works-G-H,M),L), sort(L,LSorted), subList(LSorted,S), length(S,K), first(S,HIni), last(S,HFin), HFin is HIni + K - 1.
...
treatResult(10,_):- %   shell('cat model',_),	
    ...
    maxConsecutiveHours(M,K), %maxima K tal que algun gangster treballa K hores consecutives -> no implementat ---- between(0,71,N), K is 72-N, gangster(G), findall(H,G works en H segun M,L), sort(L,Lsorted), subList(Lsorted,S), length(S,K)
    %S = [Hini,...], last(S,HFin), HFin is Hini + K - 1.
    %sublist(L,S):- append(_,L1,L), append(S,_,L1)
