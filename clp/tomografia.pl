...
matrixByRows([],_,[]).
matrixByRows(L,NumCols,[Row|MatrixByRows]):- length(Row,NumCols), append(Row,Rest,L), matrixByRows(Rest,NumCols,MatrixByRows).

declareConstraints([],[]).
declareConstraints([Row|Matrix],[S|Sum]):- sum(Row,#=,S), declareConstraints(Matrix,Sum).
